...
class ChangeIP extends StatelessWidget {
  ...
  setHostConfToSP(var key, val) async {
    ...
    //Function to Login
    appLogin() async {
      var url = "http://$newIP/cgi-bin/linuxlogin.py?username=$uname&password=$passwd";
      print(url);
...
class HelpDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                Text("1) When Changing the Host, Make Sure that IP you connect must be reachable and have CGI Files\n"),
                Text("2) Not Liking Light Theme, Turn on dark Mode in you mobile\n"),
                Text("3) Run 'clear' command to clear the Screen\n"),
                Text("4) Use 'tput' command with 'setaf' and 'setbf' subcommand to change colors\n"),
                Text("4) Open Side Drawer to see History and to see output of any command in history Click on it"),
              ],
            ),
          );
  }
